...
class NPC(pygame.sprite.Sprite):
    def __init__(self, level, start_pos):
        ...
        self.current_pos = cfg.Coord( *tuple_utils.ScaleTuple(start_pos,1.0) )
    ...
    def CalculateRect(self):
        self.rect.topleft = utils.TilePosToPlayerPos(utils.TileCoordToPos(self.current_pos))
    ...
    def Move(self,keydown):
        ...
        if self.level.levelmap[int(self.target_pos.y)][int(self.target_pos.x)] in cfg.tiles["unwalkable"]:
            self.target_pos = self.current_pos
    ...
    def update(self):
        ...
        # Do a random walk to start with
        else:
